    ...
    public class HuffmanTreeBuilder
    {
        ...
        public  HuffmanNode BuildTree(List<HuffmanNode> nodes)
        {
            ...
            while (minHeap.Count > 1)
            {
                ...
                HuffmanNode leftChild = minHeap.ExtractMin();
                HuffmanNode rightChild = minHeap.ExtractMin();

                // Create a new internal node with these two nodes as children
                HuffmanNode internalNode = new HuffmanNode(byte.MaxValue, leftChild.Frequency + rightChild.Frequency);
                internalNode.size = leftChild.size + rightChild.size + 1;
                internalNode.LeftChild = leftChild;
                internalNode.RightChild = rightChild;

                // Enqueue the new internal node back into the MinHeap
